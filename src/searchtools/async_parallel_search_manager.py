...
class AsyncParallelSearchManager:
    ...
    def __init__(self, enable_rerank: bool = None, rerank_config: RerankConfig = None):
        ...
        # 如果没有提供rerank_config，从全局配置创建
        if enable_rerank and rerank_config is None:
            rerank_config = RerankConfig(
                relevance_weight=getattr(config, 'rerank_relevance_weight', 0.40),
                authority_weight=getattr(config, 'rerank_authority_weight', 0.30),
                recency_weight=getattr(config, 'rerank_recency_weight', 0.20),
                quality_weight=getattr(config, 'rerank_quality_weight', 0.10)
            )
    ...
    def rerank_results(self, results: List[SearchResult], query: str) -> List[SearchResult]:
        ...
        Returns:
            重排序后的结果列表
        """
        if not self.enable_rerank or not self.rerank_engine:
            logger.info("[AsyncParallelSearch] Rerank disabled, returning original order")
            return results
